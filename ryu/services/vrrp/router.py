...
class Timer(object):
    ...
    def start(self, interval):
        ...
        if self._thread:
            self.cancel()
        self._event.clear()
        self._thread = gevent.spawn_later(0, self._timer, interval)
...
class VRRPRouter(app_manager.RyuApp):
    ...
    def state_change(self, new_state):
        ...
        state_changed = vrrp_event.EventVRRPStateChanged(
            self.name, self.interface, self.config.vrid, self.config.is_ipv6,
            old_state, new_state)
...
class VRRPV2StateMaster(VRRPState):
    ...
    def adver(self, ev):
        vrrp_router = self.vrrp_router
        vrrp_router.logger.debug('%s adver', self.__class__.__name__)
...
class VRRPV2StateBackup(VRRPState):
    def master_down(self, ev):
        vrrp_router = self.vrrp_router
        vrrp_router.logger.debug('%s master_down', self.__class__.__name__)
