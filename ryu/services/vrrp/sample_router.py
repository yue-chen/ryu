...
class RouterBase(app_manager.RyuApp):
    def __init__(self, *args, **kwargs):
        ...
        # TODO:XXX race
        vrrp_api.vrrp_register(self, self.instance_name, self.name)
...
class RouterIPV4(RouterBase):
    def __init__(self, *args, **kwargs):
        ...
        a = arp.arp_ether_ip(arp.ARP_REQUEST, src_mac,
                             self.config.primary_ip_address, src_mac, 0)
    ...
    def _arp_reply_packet(self, arp_req_sha, arp_req_spa):
        src_mac = vrrp.vrrp_ipv4_src_mac_address(self.config.vrid)
        e = ethernet.ethernet(arp_req_sha, src_mac, ether.ETH_TYPE_ARP)
        a = arp.arp_ether_ip(arp.ARP_REPLY, src_mac,
                             self.config.primary_ip_address,
        ...
        for field in msg.match.fields:
            ...
            if header == ofproto.OXM_OF_IN_PORT:
                if field.value != self.interface.port_no:
                    return
            elif header == ofproto.OXM_OF_ETH_DST:
                if field.value != mac_lib.BROADCAST:
                    return
            elif header == ofproto.OXM_ETH_DST:
                if field.value != dst_mac:
                    return
            elif header == ofproto.OXM_OF_ETH_TYPE:
                if not (field.value == ether.ETH_TYPE_ARP or
                        (self.interface.vlan_id is not None and
                         field.value == ether.ETH_TYPE_8021Q)):
                    return
            elif header == ofproto.OXM_OF_VLAN_VID:
                if field.value != self.interface.vlan_id:
                    return
            elif header == ofproto.OXM_OF_ARP_OP:
                if field.value != arp.ARP_REQUEST:
                    return
            elif header == ofproto.OXM_OF_ARP_SHA:
                arp_sha = field.value
            elif header == ofproto.OXM_OF_ARP_SPA:
                arp_spa = field.value
            elif header == ofproto.OXM_OF_ARP_THA:
                if field.value != dst_mac:
                    return

        if arp_sha is None or arp_spa is None:
            self.logger.debug('malformed arp request? arp_sha %s arp_spa %s',
                              arp_sha, arp_spa)
            return

        data = self._arp_reply_packet(arp_sha, arp_spa)
        dp = self._get_dp()
        if dp is None:
            return
        utils.dp_packet_out(dp, self.interface.port_no, data)

    def _send_garp(self, dp):
        dp = self._get_dp()
        if dp is None:
            return
        utils.dp_packet_out(dp, self.interface.port_no, self.garp_packet.data)
