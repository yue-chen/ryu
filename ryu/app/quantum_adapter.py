...
class OVSSwitch(object):
    def __init__(self, dpid, nw, ifaces, q_api, ctrl_addr):
        # TODO: clean up
    ...
    @staticmethod
    def create(dpid, nw, ifaces):
        token = None
        if FLAGS.quantum_auth_strategy:
            token = _get_auth_token()
        q_api = _get_quantum_client(token)
        return OVSSwitch(dpid, nw, ifaces,
                         q_api, FLAGS.quantum_controller_addr)
    ...
    def set_ovsdb_addr(self, dpid, ovsdb_addr):
        ...
        if self.ovs_bridge is None:
            ...
            for port in self.ports.values():
                ...
                self.update_port(port.ofport, True)
    ...
    def update_port(self, port_no, add):
...
class QuantumAdapter(app_manager.RyuApp):
    ...
    def _get_ovs_switch(self, dpid, create=True):
        ...
        if not ovs_switch:
            if create:
                ovs_switch = OVSSwitch.create(dpid, self.nw, self.ifaces)
                self.dps[dpid] = ovs_switch
        else:
            LOG.debug('ovs switch %s is already known', dpid)
        return ovs_switch

    def _port_handler(self, dpid, port_no, add):
        ovs_switch = self._get_ovs_switch(dpid)
        if ovs_switch:
            ovs_switch.update_port(port_no, add)
        else:
            LOG.warn('unknown ovs switch %s %s %s\n', dpid, port_no, add)
    ...
    def dp_handler(self, ev):
        ...
        if ev.enter_leave:
            for port in ev.ports:
                ovs_switch.update_port(port.port_no, True)
        else:
    ...
    def quantum_iface_set_handler(self, ev):
        ...
        if not ovs_switch:
            return
        ovs_switch.update_port(ofport, True)
