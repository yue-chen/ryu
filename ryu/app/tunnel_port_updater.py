    ...
    def update_remote(self, remote_dpid, remote_ip):
        if self.dpid == remote_dpid:
            ...
            for tp in list(self.tunnels.values()):
                ...
                self._api_delete(tp.port_no)

                new_tp = self.add_tunnel_port(tp.remote_dpid, tp.remote_ip)
                self._api_update(new_tp.port_no, tp.remote_dpid)
            return
        ...
        for tp in list(self.tunnels.values()):
            if tp.remote_dpid == remote_dpid:
                ...
                new_tp = self.add_tunnel_port(remote_dpid, remote_ip)
                self._api_update(new_tp.port_no, remote_dpid)
...
class TunnelPortUpdater(app_manager.RyuApp):
    ...
    def conf_switch_set_handler(self, ev):
        ...
        if ev.key == cs_key.OVS_TUNNEL_ADDR:
            for tunnel_dpid in self.tunnel_dpset.values():
                tunnel_dpid.update_remote(ev.dpid, ev.value)
