...
def atom_from_string(base, value_string, symtab=None):
    ...
    elif type_ == ovs.db.types.UuidTypes:
        if value_string[0] == "@":
...
class VSCtlContext(object):
    ...
    def __init__(self, idl, txn, ovsrec_open_vswitch):
...
class VSCtlCommand(object):
    def __init__(self, command, args, options=None):
...
class VSCtl(object):
    ...
    def _do_vsctl(self, idl_, commands):
        ...
        if self.wait_for_reload:
            ovs.increment(vswitch_idl.OVSREC_OPEN_VSWITCH_COL_NEXT_CFG)

        # TODO:XXX
        # symtab = ovsdb_symbol_table_create()
        ctx = VSCtlContext(idl_, txn, ovs)
        for command in commands:
            if not command._run:
                continue
    ...
    def _pre_add_port(self, ctx, columns):
        schema_helper = self.schema_helper
        schema_helper.register_columns(
            vswitch_idl.OVSREC_TABLE_PORT,
            [vswitch_idl.OVSREC_PORT_COL_NAME,
    ...
    def _pre_get_table(self, ctx, table_name):
    ...
    def _pre_get_column(self, ctx, table_name, column):
...
if __name__ == '__main__':
    ...
    location = sys.argv[1]
    prefix = sys.argv[2]
    schema_helper = idl.SchemaHelper(location, prefix)
