...
class VSCtl(object):
    ...
    def _list_ifaces_verbose(self, ctx, datapath_id,
                             ofport=None, port_name=None):
        ctx.populate_cache()
        ...
        iface_cfgs = []
        if ofport is None and port_name is None:
            ...
            if ofport is not None:
                for vsctl_port in br.ports:
                    iface_cfgs.extend(
                        ...
                        for vsctl_iface in vsctl_port.ifaces
                        if vsctl_iface.iface_cfg.ofport[0] == ofport)

            # When port is created, ofport column might be None.
            # So try with port name if it happended
            if not iface_cfgs and port_name is not None:
                for vsctl_port in br.ports:
                    iface_cfgs.extend(
                        self._iface_to_dict(vsctl_iface.iface_cfg)
                        for vsctl_iface in vsctl_port.ifaces
                        if vsctl_iface.iface_cfg.name[0] == port_name)
    ...
    def _cmd_list_ifaces_verbose(self, ctx, command):
        ...
        if len(command.args) >= 2:
            ofport = int(command.args[1])
        if len(command.args) == 3:
            port_name = command.args[2]
        iface_cfgs = self._list_ifaces_verbose(ctx, datapath_id,
                                               ofport, port_name)
